...
@FeignClient(name = "memberLoginClient", url = "${gateway.base-url}")
public interface MemberLoginFeignClient {

    @PostMapping("/auth-server/login")
    public ResponseEntity<AccessTokenResponse> doLogin(@Valid @RequestBody MemberLoginRequest memberLoginRequest);
